    ...
    <div class="pagetitle">
      <h1>Tabel Nota Setting</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item">Table Data</li>
          <li class="breadcrumb-item active">Data Nota Setting</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <pre></pre>
              <a href="<?= base_url('/nota/inputsetting') ?>" class="btn btn-success mb-3 text-white">Tambah Data</a>
              <a href="<?= base_url('nota/deletedsetting') ?>" class="btn btn-warning mb-3 text-black">Deleted Data</a>
              ...
              <table class="table datatable">
                ...
                <tbody>
                  <?php $ms = 1; foreach ($love as $key => $value) { ?>
                    <tr>
                      <td><?= $ms++ ?></td>
                      <td><img src="<?= base_url('assets/img/' . $value->logo); ?>" width="45px" class="rounded-circle"></td>
                      <td><?= $value->title ?></td>
                      <td><?= $value->notelp ?></td>
                      <td><?= $value->alamat ?></td>
                     
                      <td>
                        <button class="btn btn-info btn-detail" data-id="<?= $value->id_notset ?>"><i class="bi bi-info-circle" style="color: white;"></i></button>
                        <div class="btn-group btn-group-action mt-2" id="action-<?= $value->id_notset ?>" style="display: none;"> 
                          <a href="<?= base_url('/nota/softsetting/' . $value->id_notset) ?>" class="btn btn-danger btn-delete"><i class="bi bi-trash"></i></a><pre></pre> <!-- hapus -->
                          <a href="<?= base_url('/nota/editsetting/' . $value->id_notset) ?>" class="btn btn-warning"><i class="bi bi-pencil" style="color: white;"></i></a> <!-- edit -->
                        </div>
                      </td>
                    </tr>
                  <?php } ?>
