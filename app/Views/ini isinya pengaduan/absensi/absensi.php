...
<pre></pre>
    ...
    <div class="accordion mt-2" id="ekskul<?= $e->id_ekskul ?>">
      <div class="accordion-item">
        ...
        onclick="openTambahAbsensi(<?= $e->id_ekskul ?>)"
  ...
  Tambah Absensi Baru
            ...
            <ul>
              <?php foreach ($e->tanggal as $t) { ?>
                <li>
                  <a href="javascript:void(0)" onclick="openDetailAbsensi(<?= $t->id_absensi ?>)">
                    <?= $t->tanggal ?>
                  </a>
                </li>
              <?php } ?>
...
<?php } ?>
<div class="modal fade" id="modalTambahAbsensi">
...
<script>
function openTambahAbsensi(id) {
  document.getElementById('id_ekskul').value = id;

  fetch('<?= base_url("absensi/siswa") ?>/' + id)
    .then(r => r.text())
    .then(html => {
