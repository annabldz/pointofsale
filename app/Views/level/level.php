      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <pre></pre>
              <a href="<?= base_url('/level/input') ?>" class="btn btn-success mb-3 text-white"onclick="return confirm('Apakah Anda yakin ingin menambah data user?')">Tambah Data Level</a>
              <?php if ($level === '1'): ?>

              <a href="<?= base_url('level/deleted') ?>" class="btn btn-warning mb-3 text-black">Deleted Level</a>
<?php endif; ?>
                ...
                <tbody>
                  <?php $ms = 1; foreach ($love as $key => $value) { ?>
                    <tr>
                      ...
                      <td>
                        <button class="btn btn-info btn-detail" data-id="<?= $value->id_level ?>"><i class="bi bi-info-circle" style="color: white;"></i></button>
                        <div class="btn-group btn-group-action mt-2" id="action-<?= $value->id_level ?>" style="display: none;"> 
                          <?php if ($level === '1'): ?>
<a href="<?= base_url('/level/soft/' . $value->id_level) ?>" class="btn btn-danger btn-delete"><i class="bi bi-trash"></i></a><pre></pre> <!-- hapus --><?php endif; ?>
                          <a href="<?= base_url('/level/edit/' . $value->id_level) ?>" class="btn btn-warning"><i class="bi bi-pencil" style="color: white;"></i></a> <!-- edit -->
                        </div>
                      </td>
                    </tr>
                  <?php } ?>
