      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <pre></pre>
              <a href="<?= base_url('/menu/input') ?>" class="btn btn-success mb-3 text-white">Tambah Data menu</a>
                ...
                <tbody>
                  <?php $ms = 1; foreach ($love as $key => $value) { ?>
                    <tr>
                      ...
                      <td><?= $value->icon ?></td>
                      <td>
  <?= $value->parent_id === NULL ? 'Tidak ada Parent' : $value->parent_id ?>
</td>

                     
                      <td>
                        <button class="btn btn-info btn-detail" data-id="<?= $value->id_menu ?>"><i class="bi bi-info-circle" style="color: white;"></i></button>
                        <div class="btn-group btn-group-action mt-2" id="action-<?= $value->id_menu ?>" style="display: none;"> 
                          <a href="<?= base_url('/menu/hapus/' . $value->id_menu) ?>" class="btn btn-danger btn-delete"><i class="bi bi-trash"></i></a><pre></pre> <!-- hapus -->
                          <a href="<?= base_url('/menu/edit/' . $value->id_menu) ?>" class="btn btn-warning"><i class="bi bi-pencil" style="color: white;"></i></a> <!-- edit -->
                        </div>
                      </td>
                    </tr>
                  <?php } ?>
